...
def deal_with_one_scene(detector_name, scene, num_keypts):
    # pcdpath = f"../data/3DMatch/fragments/{scene}/"
    pcdpath = f"./numpy_3dmatch_003/{scene}/"
    gtpath = f'../geometric_registration/gt_result/{scene}-evaluation/'
    gtLog = loadlog(gtpath)

    # register each pair
    num_frag = len([filename for filename in os.listdir(pcdpath) if filename.endswith('npy')])
    # num_frag = len(os.listdir(pcdpath))
    # print(f"Start Evaluate Repeatability for {scene}")
    ...
    for id1 in range(num_frag):
        for id2 in range(id1 + 1, num_frag):
                ...
                #  read keypts from keypoints_baseline/{detector_name}_{num_keypts}/{scene}/
                source_keypts = np.fromfile(os.path.join('keypoints_baseline', f'{detector_name}_{num_keypts}/{scene}/{id1}.bin'), dtype=np.float32).reshape([-1, 3])
                target_keypts = np.fromfile(os.path.join('keypoints_baseline', f'{detector_name}_{num_keypts}/{scene}/{id2}.bin'), dtype=np.float32).reshape([-1, 3])
...
def calculate_repeatability(num_keypts):
    ...
    for scene in scene_list:
        repeatability = deal_with_one_scene(scene=scene, detector_name=detector_name, num_keypts=num_keypts)
