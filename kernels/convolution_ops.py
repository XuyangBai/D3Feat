...
def KPConv_ops(query_points,
    ...
    if KP_influence == 'constant':
        ...
        all_weights = tf.maximum(1 - tf.sqrt(sq_distances + 1e-10) / (2 * KP_extent), 0.0)
        all_weights = tf.transpose(all_weights, [0, 2, 1])
    ...
    # normalization term.
    neighbor_features_sum = tf.reduce_sum(neighborhood_features, axis=-1)
    neighbor_num = tf.reduce_sum(tf.cast(tf.greater(neighbor_features_sum, 0.0), tf.float32), axis=-1)
    neighbor_num = tf.maximum(neighbor_num, 1)
    output_features = tf.divide(output_features, tf.expand_dims(neighbor_num, 1))
...
def KPConv_deform_ops(query_points,
    ...
    if KP_influence == 'constant':
        ...
        # Influence decrease linearly with the distance, and get to zero when d = KP_extent.
        all_weights = tf.maximum(1 - tf.sqrt(new_sq_distances + 1e-10) / KP_extent, 0.0)
        all_weights = tf.transpose(all_weights, [0, 2, 1])
