...
def context_norm(x, use_context_norm=True, training=True):
    """
    This tensorflow operation compute a context normalization.
    > x = [n1, d] features matrix
    >> output = [n1, d] normalized, scaled, offset features matrix
    """
    if use_context_norm:
        ...
        out = tf.nn.batch_normalization(x, mean, variance, None, None, var_eps, training=training)
        return 
    else:
        # Just add biases
        beta = tf.Variable(tf.zeros([x.shape[-1]]), name='offset')
...
def last_unary_block(layer_ind, inputs, features, radius, fdim, config, training):
    """
    Block performing a simple 1x1 convolution
    """
    w = weight_variable([int(features.shape[1]), 32])
...
def assemble_CNN_blocks(inputs, config, dropout_prob):
    ...
    for block_i, block in enumerate(config.architecture):
        ...
        with tf.variable_scope('layer_{:d}/{:s}_{:d}'.format(layer, block.replace("_deformable", ""), block_in_layer)):
